...
$message='<font color="red">Both Passwords are not Matched</font>';	
	...
	$query1=mysqli_query($con,"SELECT * from collection_center WHERE unique_id='$username' AND  email='$email'  LIMIT 1") or die (mysqli_error());
	$count_query1=mysqli_num_rows($query1);        
    $query2=mysqli_query($con,"SELECT * from members WHERE username='$username' AND  email='$email'  LIMIT 1") or die (mysqli_error());
	...
	if($count_query1==0 && $count_query2==0){
	    ...
	    $npass=md5($newpass);
        if ($count_query1 > 0) {
            mysqli_query($con,"update collection_center set 
            password='$npass' where unique_id='$username' AND  email='$email' limit 1") or die (mysqli_error($con));
            
        } elseif ($count_query2 > 0) {
