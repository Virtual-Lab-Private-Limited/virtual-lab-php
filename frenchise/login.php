...
if (isset($_POST['username'])) {
  ...
  if ((!$username) || (!$pass)) {
    ...
    //secure the data
    $pass = md5($pass);
    $query = mysqli_query($con, "SELECT * from frenchises WHERE username='$username' AND  password='$pass' and active='1' LIMIT 1") or die(mysqli_error($con));
    $count_query1 = mysqli_num_rows($query);
    $query2 = mysqli_query($con, "SELECT * from members WHERE username='$username' AND  password='$pass' and status='Active' and member_type=5 LIMIT 1") or die(mysqli_error($con));
...
<head>
  ...
  <title>Virtual Lab | Frenchise Dashboard </title>
  <!-- General CSS Files -->
