...
while($info=mysqli_fetch_array($labs)){
 $lab_ratio=$info['services_charges'];
}
	
	$company_profit=$total_amount*$lab_ratio/100;
	$lab_profit=$total_amount-$company_profit;
...
$company_profits=mysqli_query($con,"select * from company_profit where bookingid='$id' limit 1") or die (mysqli_error());
$count_company=mysqli_num_rows($company_profits);
...
if($count_lab>0){
	...
	header("location:".$baseurl."bylab.html");
}
