...
if (!empty($_POST["value_based"])) {
    ...
    foreach ($result as $index => $value) {
        ...
        if($count_results > 0) {
            mysqli_query($con, "update value_based_result set value = '$value', priority='$priority[$index]' where bdid='$bdid' and testid='$tid[$index]' ") or die(mysqli_error($con));
...
if (!empty($_POST["publish_value_based"])) {
    ...
    mysqli_query($con, "update booking_details set status='Complete', runtime_status='Report generated', labid='$session_labid', result_date='$today', remarks='$remarks' where id='$bdid'") or die(mysqli_error());
...
if (isset($_POST["publish_culture"])) {
    ...
    mysqli_query($con, "update booking_details set status='Complete',runtime_status='Report generated',labid='$session_labid', result_date='$today', remarks='$remarks' where id='$bookingdetailid'") or die(mysqli_error());
    push_notification($pid, $con);
...
if (isset($_POST['radiology'])) {
    ...
    mysqli_query($con, "update booking_details set status='Complete',runtime_status='Report generated',labid='$session_labid', result_date='$today', remarks='$remarks' where id='$bdid'") or die(mysqli_error());
   	//email($booking_no, $session_email, $bdid);
    push_notification($pid, $con);
    echo '<script language="javascript">window.location = "' . $baseurl . 'report/' . $bdid . '.html"</script>';
...
function email($bookingno, $email, $bdid){
    $to = "$email";
    ...
    <html>
    <head>
    <title>Dear Sir/Madam</title>
    </head>
    <body>
    <h4>Dear Sir/Madam</h4>
    <p>Thank you for choosing Virtual Lab. Your report against booking no ".$bookingno." is ready to view/download at https://virtuallab.com.pk/reports/pdf.php?id=".$bdid.". 
...
$bds = mysqli_query($con, "select *  from bookings where id='$bid'") or die(mysqli_error());
while ($bd = mysqli_fetch_array($bds)) {
    $booking_no = $bd['bookingno'];
