...
$message='<font color="red">Both Passwords are not Matched</font>';	
	...
	$query=mysqli_query($con,"SELECT * from frenchise_doctor WHERE firstname='$username' AND  pmdc='$pmdc'  LIMIT 1") or die (mysqli_error());
	$count_query=mysqli_num_rows($query);
	
	print_r($count_query);

	if($count_query==0){
        $message='<font color="red"><p align="center">Invaild Username/PMDC Number</p></font>';
     
	} else {
	    $npass=md5($newpass);

        mysqli_query($con,"update frenchise_doctor set 
        password='$npass' where firstname='$username' AND  pmdc='$pmdc' limit 1") or die (mysqli_error($con));
...
<input type="text" class="form-control" name="pmdc" value="">
                      </div>
                    </div>
                    <div class="form-group">
