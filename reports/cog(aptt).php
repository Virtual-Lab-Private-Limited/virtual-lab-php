<?php
    ...
    $results=mysqli_query($con,"select * from  coagulation_result where bdid='$id' ") or die (mysqli_error());
    ...
    while($row=mysqli_fetch_array($results)){
        ...
        $inr = ($row['value']/$row['control']);
        $inr = number_format((float)$inr, 2, '.', '');
        
        if( ! empty( $row['value']) ) {
            $output .= '<tr><td>'.$test_title.'</td><td>'.$row['value'].'</td><td>'.$row['control'].'</td><td>'.$inr.'</td><td>'.$unit.'</td>';
            $records = mysqli_query($con, "select * from coagulation_result where pid='$pt' and testid='$testid' and id <='$id' order by id DESC  limit 3 ") or die(mysqli_error());
            $count_row=mysqli_num_rows($records);
            ...
            while($record=mysqli_fetch_array($records)){ 
                ...
                $inr = ($record['value']/$record['control']);
                $inr = number_format((float)$inr, 2, '.', '');
...
$style = array(
    ...
    // $pdf->write1DBarcode($bookingno, 'C39', 100, 70, 20, 10, 0.4, $style, 'N');
...
$content = '';  
     ...
     $content .= '  
      ...
      <table class="table table-md">  
        ...
        <th width="20%" style="font-weight:bold">Test</th>
        <th class="text-left" width="15%" style="font-weight:bold">Patient Value</th>
        <th class="text-left" width="15%" style="font-weight:bold">Control</th>    
        <th class="text-left" width="15%" style="font-weight:bold">INR</th>
    ...
    $row3 .= '
        ...
        if(count($bookings) == 1) { 
            ...
            //$pdf->write1DBarcode($bookings[0], 'C39', 145, 70, 20, 10, 0.4, $style, 'N');
            //$pdf->SetXY(148, 80);
            ...
            //$pdf->writeHTMLCell(0, 0, '', '', $date, 0, 1, 0, true, '', true);
            $content .= '<th class="text-left" width="15%" style="font-weight:bold"> INR</th>';
            ...
            $chunks = array_chunk($split, 3);
            $result = array_map(function($chunk) { return implode(' ', $chunk); }, $chunks);
            $date = '<h6 style="font-height:10px">'. $result[1].'</h6> ';
