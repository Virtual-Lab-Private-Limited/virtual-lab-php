<?php
    ...
    $results=mysqli_query($con,"select * from  pcr_qualitative_result where bdid='$id' ") or die (mysqli_error());
    ...
    while($row=mysqli_fetch_array($results)){
        ...
        if( ! empty( $row['result']) ) {
            $output .= '<tr><td>'.$test_title.'</td><td>'.$row['result'].'</td>';
            $records = mysqli_query($con, "select * from pcr_qualitative_result where ptid='$pt' and testid='$testid' and id <='$id' order by id DESC  limit 3 ") or die(mysqli_error());
 ...
 $content = '';  
    ...
    $content .= '  
      ...
      <table class="table table-md">  
        ...
        <th width="55%" style="font-weight:bold">Test</th>
        <th class="text-left" width="20%" style="font-weight:bold">Result</th>';
    ...
    $row3 .= '
        ...
        if(count($bookings) == 1){ 
            ...
            //$pdf->writeHTMLCell(0, 0, '', '', $date, 0, 1, 0, true, '', true);
            $content .= '<th class="text-left" width="15%" style="font-weight:bold"> Result</th>';
