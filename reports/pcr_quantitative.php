<?php
    ...
    $results=mysqli_query($con,"select * from  pcr_quantitative_result where bdid='$id' ") or die (mysqli_error());
    ...
    while($row=mysqli_fetch_array($results)){
        ...
        if( ! empty( $row['viralload']) ) {
                        
            
            $output .= '<tr><td>'.$test_title.'</td><td>'.$row['viralload'].'</td><td>'.$unit.'</td><td>'.$row['result'].'</td>';
            $records = mysqli_query($con, "select * from pcr_quantitative_result where ptid='$pt' and testid='$testid' and id <='$id' order by id DESC  limit 3 ") or die(mysqli_error());
    ...
    // $pdf->write1DBarcode($bookingno, 'C39', 110, 70, 20, 10, 0.4, $style, 'N');
    // $pdf->SetXY(113, 80);
...
$row3 = '';
 ...
 $content = '';  
      ...
      <table class="table table-md">  
        ...
        <th width="30%" style="font-weight:bold">Test</th>
        <th class="text-left" width="15%" style="font-weight:bold">Viralload</th>
        <th class="text-left" width="10%" style="font-weight:bold">Unit</th>
        <th class="text-left" width="15%" style="font-weight:bold">Results</th>';
    $row1 .= '
        <tr style="background-color: #ddd; font-size: 6px"> 
        <td colspan="1"></td>
        <th class="text-left" scope="col">'.$result[0].'</th><td colspan="2"></td>';
    $row2 .= '
        <tr style="background-color: #ddd; font-size: 6px"> 
        <td colspan="1"></td>
        <th class="text-left" scope="col">'.$result[1].'</th><td colspan="2"></td>';
   
    $row3 .= '
        <tr style="background-color: #ddd; font-size: 6px; "> 
        <td colspan="1"></td>
        <th class="text-left" scope="col">'.$bookingno.'</th><td colspan="2"></td>';
   
        
        if(count($bookings) == 1){ 
            
            // $pdf->write1DBarcode($bookings[0], 'C39', 140, 70, 20, 10, 0.4, $style, 'N');
            // $pdf->SetXY(143, 80);
            ...
            // $pdf->writeHTMLCell(0, 0, '', '', $date, 0, 1, 0, true, '', true);
            $content .= '<th class="text-left" width="17%" style="font-weight:bold"> Viraload</th>';
            $row1 .= '<th class="text-left" width="17%" > '.$result[0].'</th>';
            $row2 .= '<th class="text-left" width="17%" > '.$result[1].'</th>';
            $row3 .= '<th class="text-left" width="17%" > '.$bookings[0].'</th>';
            ...
            // $pdf->write1DBarcode($bookings[1], 'C39', 170, 70, 20, 10, 0.4, $style, 'N');
            // $pdf->SetXY(173, 80);
            ...
            $content .= '<th class="text-left" width="15%" style="font-weight:bold" >Viraload</th>';
