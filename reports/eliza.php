<?php
    ...
    $results=mysqli_query($con,"select * from  eliza_result where bdid='$id' ") or die (mysqli_error());
    ...
    while($r=mysqli_fetch_array($tests)){
        ...
        if( ! empty( $row['patient_value']) ) {
            $output .= '<tr><td>'.$test_title.'</td><td>'.$row['patient_value'].'</td><td>'.$row['result'].'</td>';
             
            $records = mysqli_query($con, "select * from eliza_result where ptid='$pt' and testid='$testid' and id <='$id' order by id DESC  limit 3 ") or die(mysqli_error());
            ...
            $i = 0;
             while($record=mysqli_fetch_array($records)){ 
                ...
                $output .= '<td >'.  $record['patient_value'].' </td>';
                $b_id = $record['bid'];
...
$style = array(
    ...
    // $pdf->write1DBarcode($bookingno, 'C39', 103, 70, 20, 10, 0.6, $style, 'N');
    // $pdf->SetXY(106, 80);
...
$row3 = '';
 ...
 $content = '';  
      ...
      <table class="table table-md">  
        ...
        <th class="text-left" width="20%" style="font-weight:bold">Patient Value</th>
    ...
    $row3 .= '
        ...
        <th class="text-left" scope="col">'.$bookingno.'</th>';
          ...
          if(count($bookings) == 1){ 
            ...
            //$pdf->write1DBarcode($bookings[0], 'C39', 137, 70, 20, 10, 0.4, $style, 'N');
            //$pdf->SetXY(140, 80);
            ...
            $content .= '<th class="text-left" width="20%" style="font-weight:bold"> Patient Value</th>';
            ...
            $content .= '<th class="text-left" width="15%" style="font-weight:bold"> Patient Value</th>';
            $content .= '<th class="text-left" width="15%" style="font-weight:bold" > Patient Value</th>';
             $row1 .= '<th class="text-left" width="20%" > '.$result[0].'</th>';
            $row2 .= '<th class="text-left" width="20%" > '.$result[1].'</th>';
            $row3 .= '<th class="text-left" width="20%" > '.$bookings[1].'</th>';
     }
         $content .=  '</tr>  ';  
          $row1 .= '</tr>';
      $row2 .= '</tr>';
